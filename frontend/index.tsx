    ...
    while (apis.length < count) {
        ...
        if (!usedNames.has(uniqueName)) {
            ...
            apis.push({
                ...
                endpoint: `${template.endpoint}/${apis.length + 1}`, // Unique endpoint path for simulation
                type: template.type,
...
const generateCodeSnippet = (api: ApiOption | null) => {
  ...
  // Use the real server URL for the code snippet, but use the base endpoint path
  const baseEndpoint = api.endpoint.substring(0, api.endpoint.lastIndexOf('/'));
  const apiUrl = `${SERVER_URL}${baseEndpoint}`;
...
const App: React.FC = () => {
  ...
  const pollVideoStatus = (operationId: string) => {
    if (videoPollingIntervalRef.current) clearInterval(videoPollingIntervalRef.current);

    videoPollingIntervalRef.current = window.setInterval(async () => {
      try {
        const res = await fetch(`${SERVER_URL}/v1/video-status/${operationId}`);
  ...
  const handleGenerate = async () => {
    ...
    try {
        const baseEndpoint = selectedApi.endpoint.substring(0, selectedApi.endpoint.lastIndexOf('/'));
        ...
        if (selectedApi.type === 'image-to-text') {
            ...
            const res = await fetch(`${SERVER_URL}${baseEndpoint}`, { method: 'POST', body: formData });
            const data = await res.json();
            ...
            const res = await fetch(`${SERVER_URL}${baseEndpoint}`, {
                method: 'POST',
  ...
  const baseEndpoint = selectedApi?.endpoint.substring(0, selectedApi.endpoint.lastIndexOf('/'));
  const apiUrl = selectedApi ? `${SERVER_URL}${baseEndpoint}` : '';
  const apiTypes: (ApiOption['type'] | 'all')[] = ['all', 'text', 'image', 'video', 'audio', 'image-to-text'];

  return (
    <div className="container">
      <header>
        <h1>Full-Stack AI API Server</h1>
        <p>Choose an API below. Your request will be sent to your real backend server, which then calls Gemini.</p>
      </header>
      ...
      <main>
        <section className="explanation-section">
          <button onClick={() => setShowExplanation(!showExplanation)} className="explanation-toggle" aria-expanded={showExplanation} aria-controls="explanation-content">
            <span>How This Full-Stack App Works</span>
            <span className={`arrow ${showExplanation ? 'up' : 'down'}`}></span>
          </button>
          {showExplanation && (
            <div id="explanation-content" className="explanation-content">
              <p><strong>This is now a real full-stack application!</strong></p>
              <ol>
                <li>Your <strong>backend server</strong> is running in a terminal, securely managing your API key.</li>
                <li>This <strong>frontend UI</strong> is running in another terminal, served to your browser.</li>
                <li>When you click "Call My API", the frontend sends a request to YOUR server (at `http://localhost:3001`).</li>
                <li>Your server then safely calls the Google Gemini API and returns the response to the frontend.</li>
              </ol>
            </div>
          )}
        </section>
          ...
          <>
            ...
            <section className="api-details-section">
              <h3>Your API Details for "{selectedApi.name}"</h3>
               <p className="note">
                <em>Note: This endpoint is now REAL and is being served by your Node.js backend.</em>
              </p>
